...
func Black(c *gin.Context) {
	...
	if errors.Is(err, service.ErrFriendNotRecord) {
		...
		logger.Warnf("[http.friend] black err: %v", err)
...
func Star(c *gin.Context) {
	...
	if errors.Is(err, service.ErrFriendNotRecord) {
		...
		logger.Warnf("[http.friend] star err: %v", err)
		app.Error(c, errno.InternalServerError)
		return
	}
	app.SuccessNil(c)
...
func Remark(c *gin.Context) {
	...
	if errors.Is(err, service.ErrFriendNotRecord) {
		...
		logger.Warnf("[http.friend] remark err: %v", err)
		app.Error(c, errno.InternalServerError)
		return
	}
	app.SuccessNil(c)
...
func Auth(c *gin.Context) {
	...
	if errors.Is(err, service.ErrFriendNotRecord) {
		...
		logger.Warnf("[http.friend] auth err: %v", err)
		app.Error(c, errno.InternalServerError)
		return
	}
	app.SuccessNil(c)
