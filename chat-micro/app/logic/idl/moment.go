...
// 点赞列表数据格式化为map
func likesToMap(likes map[uint32][]uint32, users map[uint32]*model.UserBase) map[uint32][]*model.LikeUser {
	ml := make(map[uint32][]*model.LikeUser)
	if len(likes) == 0 {
		return ml
	}
