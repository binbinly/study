...
type IFriend interface {
	// 好友信息
	FriendInfo(ctx context.Context, userID, friendID uint32) (*model.FriendInfo, error)
	// 我的好友
	FriendMyAll(ctx context.Context, userID uint32) ([]*model.UserBase, error)
	// 我的指定好友
	FriendMyListByIds(ctx context.Context, userID uint32, ids []uint32) ([]*model.UserBase, error)
	// 我的标签好友
	FriendMyListByTagID(ctx context.Context, userID, tagID uint32) ([]*model.UserBase, error)
	// 设置黑名单
...
func (s *Service) FriendInfo(ctx context.Context, userID, friendID uint32) (info *model.FriendInfo, err error) {
	...
	tags := make([]string, 0)
	if f.Tags != "" {
		tags, err = s.repo.GetTagNamesByIds(ctx, userID, util.SliceTouInt32(strings.Split(f.Tags, ",")))
		if err != nil {
...
func (s *Service) friendUserList(ctx context.Context, friends []*model.FriendModel) (list []*model.UserBase, err error) {
	...
	list = make([]*model.UserBase, 0, len(users))
	for _, u := range users {
...
func (s *Service) FriendSetBlack(ctx context.Context, userID, friendID uint32, isBlack int8) error {
	f, err := s.friendInfo(ctx, userID, friendID)
	if err != nil {
		return err
	}
	f.IsBlack = isBlack
...
func (s *Service) FriendSetRemarkTag(ctx context.Context, userID, friendID uint32, nickname string, tags []string) error {
	...
	if len(tags) > 0 {
		...
		f.Tags = util.SliceUInt32ToString2(tagIds)
	}
	f.Nickname = nickname
	return s.repo.FriendSave(ctx, f)
}
...
//friendInfo
func (s *Service) friendInfo(ctx context.Context, uid, id uint32) (*model.FriendModel, error) {
	// 好友关系详情
