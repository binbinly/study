...
type IGroup interface {
	...
	// 我的群列表
	GroupMyList(ctx context.Context, userID uint32) ([]*model.GroupList, error)
	// 群详情
	GroupInfo(ctx context.Context, userID, groupID uint32) (*model.GroupInfo, error)
	// 群成员
	GroupUserAll(ctx context.Context, userID, groupID uint32) ([]*model.UserBase, error)
	// 退出群
	GroupUserQuit(ctx context.Context, userID, groupID uint32) error
	// 踢出群
	GroupKickOffUser(ctx context.Context, myID, groupID, toID uint32) error
	// 邀请入群
	GroupInviteUser(ctx context.Context, myID, groupID, toID uint32) error
	// 加入群
	GroupJoin(ctx context.Context, userID, groupID uint32) error
}
...
func (s *Service) GroupCreate(ctx context.Context, userID uint32, ids []uint32) error {
	...
	// 好友id列表
	userIDs := make([]uint32, 0, len(friends))
	...
	// 创建群组成员
	if err = s.repo.GroupUserBatchCreate(ctx, tx, s.buildGroupUser(userID, groupID, users)); err != nil {
		tx.Rollback()
	...
	// 发送消息
	if err = s.pushUserIds(ctx, uids, message.EventChat, &message.Chat{
		From: &message.Sender{
			ID:     u.ID,
			Name:   uname,
			Avatar: u.Avatar,
		...
		return errors.Wrapf(err, "[service.group] push")
	}
	return nil
}
...
func (s *Service) sendMessage(ctx context.Context, params *sendParams) (err error) {
	...
	for _, gUser := range params.gUsers {
		...
		if err := s.pushUserIds(ctx, []uint32{gUser.UserID}, message.EventChat, &message.Chat{
			From:     f,
			To:       t,
			ChatType: model.MessageChatTypeGroup,
			Type:     model.MessageTypeSystem,
			...
			logger.Warnf("[service.group] push uid: %v, err: %v", gUser.UserID, err)
		}
	}
	return nil
}
...
//buildGroupUser
func (s *Service) buildGroupUser(userID, groupID uint32, friends []*model.UserModel) []*model.GroupUserModel {
