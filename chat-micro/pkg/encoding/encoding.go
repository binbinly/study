...
func Marshal(e Encoding, v interface{}) (data []byte, err error) {
	...
	if reflect.ValueOf(v).Elem().Kind() == reflect.String {// 字符串基础类型不用 encode
		return []byte(reflect.ValueOf(v).Elem().String()), nil
	}
...
func Unmarshal(e Encoding, data []byte, v interface{}) (err error) {
	...
	if reflect.ValueOf(v).Elem().Kind() == reflect.String {// 字符串基础类型不用 decode
		return nil
	}
