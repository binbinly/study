...
func (s *Service) GroupKickOffUser(ctx context.Context, myID, groupID, toID uint32) (err error) {
	...
	// 目标用户信息
	toUser, err := s.GetUserByID(ctx, toID)
	if err != nil {
		return err
	}
...
// groupName 获取群组默认名称
func (s *Service) groupName(username string, friends []*base.UserInfo) string {
...
func (s *Service) sendMessage(ctx context.Context, params *sendParams) (err error) {
	...
	// 我的用户详情
	u, err := s.GetUserByID(ctx, params.userID)
	if err != nil {
...
func (s *Service) myGroupName(my *base.UserInfo, users []*model.GroupUserModel) string {
	name := my.Username
	if my.Nickname != "" {
		name = my.Nickname
	}
