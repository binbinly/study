...
type IGroupUser interface {
	...
	// 批量创建
	GroupUserBatchCreate(ctx context.Context, tx *gorm.DB, userID, groupID uint32, friends []*base.UserInfo) (err error)
	// 修改群成员昵称
...
func (r *Repo) GroupUserCreate(ctx context.Context, user *model.GroupUserModel) (err error) {
	...
	// 删除缓存
	err = r.groupUserAllCache.DelCache(ctx, user.GroupID)
	if err != nil {
...
func (r *Repo) GroupUserBatchCreate(ctx context.Context, tx *gorm.DB, userID, groupID uint32, friends []*base.UserInfo) (err error) {
...
func (r *Repo) GroupUserUpdateNickname(ctx context.Context, userID, groupID uint32, nickname string) error {
	...
	// 删除缓存
	err = r.groupUserAllCache.DelCache(ctx, groupID)
	if err != nil {
		return errors.Wrap(err, "[repo.group_user] delete all cache")
	}
	// 删除缓存
	err = r.groupUserCache.DelCache2(ctx, userID, groupID)
	if err != nil {
...
func (r *Repo) GetGroupUserByID(ctx context.Context, userID, groupID uint32) (info *model.GroupUserModel, err error) {
	...
	getDataFn := func() (interface{}, error) {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			err = r.groupUserCache.SetCacheWithNotFound2(ctx, userID, groupID)
			if err != nil {
...
func (r *Repo) GroupUserAll(ctx context.Context, groupID uint32) (all []*model.GroupUserModel, err error) {
	...
	// 从cache获取
	all, err = r.groupUserAllCache.GetCache(ctx, groupID)
	if err != nil {
		if err == cache.ErrPlaceholder {
	...
	getDataFn := func() (interface{}, error) {
		...
		// set cache
		err = r.groupUserAllCache.SetCache(ctx, groupID, data)
		if err != nil {
