...
func (r *Repo) GetTagsByUserID(ctx context.Context, userID uint32) (list []*model.UserTag, err error) {
	...
	// 从cache获取
	list, err = r.tagCache.GetCache(ctx, userID)
	if err != nil {
	...
	getDataFn := func() (interface{}, error) {
		...
		err = r.db.WithContext(ctx).Model(&model.UserTagModel{}).Where("user_id = ? ", userID).Order(orm.DefaultOrder).Scan(&data).Error
		if err != nil {
			return nil, errors.Wrapf(err, "[repo.tag] query db err")
		}

		// set cache
		err = r.tagCache.SetCache(ctx, userID, data)
		if err != nil {
...
func (r *Repo) TagBatchCreate(ctx context.Context, tags []*model.UserTagModel) (ids []uint32, err error) {
	...
	// 删除缓存
	err = r.tagCache.DelCache(ctx, tags[0].UserID)
	if err != nil {
