...
class chat {
  constructor(arg) {
    ...
    if (user) {
      ...
      this.user = {id:user.id, name:user.name,avatar:user.avatar}
    } else {
      this.user = {}
    }
    // 初始化聊天对象
  ...
  reconnect() {
    ...
    if (this.reconnectCount >= 3) {//自动重连次数限制，超过后弹窗
  ...
  onMessage(data) {
    ...
    switch (res.event) {
      ...
      case 'recall': // 撤回消息
        ...
        break
      case 'notify': // 通知
        if (res.data.type && res.data.type == 'apply') {// 申请加好友通知
  ...
  async handleOnMessage(message) {
    // 添加消息记录到本地存储中
    if (message.chat_type === $const.CHAT_TYPE_USER) {//单聊，接受者就是当前用户
