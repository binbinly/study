...
func TarWrite(files []*File) (buf *bytes.Buffer, err error) {
	...
	for _, file := range files {
		...
		if err = tw.WriteHeader(hdr); err != nil {
			return nil, errors.Wrapf(err, "write header")
		}
		// writer写入内容
		if _, err = tw.Write(file.Body); err != nil {
			return nil, errors.Wrapf(err, "write body")
		}
	}
	return
}
...
func TarRead(path string) ([]*File, error) {
	...
	for {
		...
		if err == io.EOF {
			...
			return nil, errors.Wrapf(err, "read header next")
		}

		// 读取数据流信息
		b, err = ioutil.ReadAll(tr)
		if err != nil {
			return nil, errors.Wrapf(err, "read body all")
		}
		file := &File{
			Name: hdr.Name,
			Body: b,
