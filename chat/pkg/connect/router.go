...
//Start 执行入口
func (e *Engine) Start(req IRequest) {
...
func (e *Engine) addIntRoute(msgID int, handlers HandlerChain) {
	e.tree[strconv.Itoa(msgID)] = handlers
}

func (e *Engine) getHandlers(req IRequest) HandlerChain {
	var event string
	if req.GetType() == MsgTypeJSON {
		event = req.GetJSONMsg().GetEvent()
	} else {
		event = strconv.Itoa(int(req.GetBinaryMsg().GetMsgID()))
	}
	handlers, ok := e.tree[event]
	if !ok {
		return nil
	}
	return handlers
...
//AddIntRoute 添加路由
func (g *RouterGroup) AddIntRoute(msgID int, handlers ...HandlerFunc) {
	handlers = g.mergeHandlers(handlers)
	g.engine.addIntRoute(msgID, handlers)
}
