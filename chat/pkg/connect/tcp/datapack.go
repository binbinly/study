...
type IDataPack interface {
	...
	Pack(msg connect.IMessage) ([]byte, error) //封包方法
	Unpack([]byte) (connect.IMessage, error)   //拆包方法
}
...
//Pack 封包方法(压缩数据)
func (d DataPack) Pack(msg connect.IMessage) ([]byte, error) {
	//创建一个存放bytes字节的缓冲
...
//Unpack 拆包方法(解压数据)
func (d DataPack) Unpack(binaryData []byte) (connect.IMessage, error) {
	...
	if int64(msg.DataLen) > connect.MaxPacketSize {
		return nil, connect.ErrLargeReceived
	}
