...
describe("锁仓合约", function () {
  it("布署锁仓合约", async function () {
    ...
    TokenTimelockInstance = await ERC20WithTokenTimelock.new(...param, {
      from: owner,
    });
  });
...
describe("测试可锁仓代币的特殊方法", function () {
  ...
  it("锁仓数量: balance()", async function () {
    assert.equal(
      ether(lockAmount),
      (await ERC20Instance.balanceOf(TokenTimelockInstance.address)).toString()
    );
  });
  //测试返回受益人
  ...
  it("解锁方法: release()", async function () {
    ...
    await TokenTimelockInstance.release({ from: owner });
    assert.equal(
      ether(lockAmount),
