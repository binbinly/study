...
func Marshal(e Encoding, v interface{}) (data []byte, err error) {
	...
	if obj.Type().Kind() == reflect.String {// 字符串基础类型不用 encode
...
func Unmarshal(e Encoding, data []byte, v interface{}) (err error) {
	...
	if obj.Type().Kind() == reflect.String {// 字符串基础类型不用 decode
		obj.SetString(string(data))
		return nil
	}
...
func isPointer(data interface{}) bool {
	...
	case reflect.Ptr, reflect.Interface, reflect.String:
		return true
	default:
		return false
	}
