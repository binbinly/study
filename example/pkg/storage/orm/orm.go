...
import (
	...
	"lib/pkg/storage/orm/gormtracing"
)
...
type Config struct {
	...
	MaxOpenConn     int
	ConnMaxLifeTime int
	SlowThreshold   int // 慢查询时长，默认500ms
}
...
func NewMySQL(c *Config) (db *gorm.DB) {
	...
	// set trace
	err = db.Use(gormtracing.New(gormtracing.WithLogResult()))
	if err != nil {
...
func gormConfig(c *Config) *gorm.Config {
	...
	if c.SlowThreshold > 0 {
		config.Logger = logger.New(
			...
			logger.Config{
				//设定慢查询时间阈值
				SlowThreshold: time.Duration(c.SlowThreshold) * time.Second, // nolint: golint
