...
const (
	PublicPemFile = "rsa_public.pem"
	PublicPKCS1PemFile = "rsa_public_pkcs1.pem"
	PrivatePemFile = "rsa_private.pem"
	PrivatePKCS1PemFile = "rsa_private_pkcs1.pem"
)
...
func TestRSAPKCS1v15(t *testing.T)  {
	...
	// rsa公钥加密
	cipherText, err := Encrypt(publicKey, origin)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Log("RSA-PKCS1v15加密内容: ", cipherTextStr)

	// rsa私钥解密
	originText, err := Decrypt(privateKey, cipherText)
	if err != nil {
		t.Fatal(err)
	}
	t.Log("RSA-PKCS1v15解密内容: ", string(originText))
	assert := internal.NewAssert(t, "TestRSAPKCS1v15")
	assert.Equal(string(originText), string(origin))
}
...
func TestRSAOAEP(t *testing.T)  {
	...
	// rsa公钥加密
	cipherText, err := EncryptOAEP(publicKey, origin, label)
	if err != nil {
		t.Fatal(err)
	}
	...
	// 转base64字符串
	cipherTextStr := base64.StdEncoding.EncodeToString(cipherText)
	t.Log("RSA-OAEP加密内容: ", cipherTextStr)

	// rsa私钥解密
	originText, err := DecryptOAEP(privateKey, cipherText, label)
	if err != nil {
		t.Fatal(err)
	}
	t.Log("RSA-OAEP解密内容: ", string(originText))
	assert := internal.NewAssert(t, "TestRSAOAEP")
	assert.Equal(string(originText), string(origin))
}
...
func TestRSASignature(t *testing.T)  {
	...
	// rsa私钥签名
	signature, err := Sign(privateKey, origin)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Log("RSA签名内容: ", signatureStr)

	//rsa公钥验签
	err = Verify(publicKey, origin, signature)
	if err != nil {
		t.Fatal(err)
	}
	t.Log("RSA签名验证成功")
...
func TestRSASignPass(t *testing.T)  {
	...
	// rsa私钥签名
	signature, err := SignPass(privateKey, origin, &SignOpts)
	if err != nil {
		t.Fatal(err)
	}
	...
	// 转base64字符串
	signatureStr := base64.StdEncoding.EncodeToString(signature)
	t.Log("RSA-PASS签名内容: ", signatureStr)

	//rsa公钥验签
	err = VerifyPass(publicKey, origin, signature, &VerifyOpts)
	if err != nil {
		t.Fatal(err)
	}
	t.Log("RSA-PASS签名验证成功")
}
