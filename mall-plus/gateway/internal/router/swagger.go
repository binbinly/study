...
// HandlerSwagger wraps `http.Handler` into `http.HandlerFunc`.
func HandlerSwagger(configFns ...func(*Config)) runtime.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request, pathParams map[string]string) {
		...
		case "chat.json":
			doc, err := ioutil.ReadFile("doc/cart/cart.swagger.json")
			if err != nil {
			...
			_, _ = w.Write(doc)
		case "market.json":
			doc, err := ioutil.ReadFile("doc/market/market.swagger.json")
			if err != nil {
				http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
				return
			}
			_, _ = w.Write(doc)
		case "member.json":
			doc, err := ioutil.ReadFile("doc/member/member.swagger.json")
			if err != nil {
				http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
				return
			}
			_, _ = w.Write(doc)
		case "order.json":
			doc, err := ioutil.ReadFile("doc/order/order.swagger.json")
			if err != nil {
				http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
				return
			}
			_, _ = w.Write(doc)
		case "product.json":
			doc, err := ioutil.ReadFile("doc/product/product.swagger.json")
			if err != nil {
				http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
				return
			}
			_, _ = w.Write(doc)
		case "seckill.json":
			doc, err := ioutil.ReadFile("doc/seckill/seckill.swagger.json")
			if err != nil {
				http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
				return
			}
