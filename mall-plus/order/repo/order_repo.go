...
//CreateOrder 创建订单
func (r *Repo) CreateOrder(ctx context.Context, tx *gorm.DB, order *model.OrderModel) (int64, error) {
...
func (r *Repo) GetOrderList(ctx context.Context, memberID int64, status, offset, limit int) (list []*model.OrderModel, err error) {
	err = r.DB.WithContext(ctx).Where("member_id=?", memberID).Preload("Items").Order(orm.DefaultOrder).
		Scopes(orderScopesStatus(status), orm.OffsetPage(offset, limit)).Find(&list).Error
	if err != nil {
