...
func NewRPCClientConn(c *ClientConfig, target string, midd grpc.UnaryClientInterceptor) *grpc.ClientConn {
	...
	conn, err := grpc.DialContext(ctx, target,
		[]grpc.DialOption{
			grpc.WithInsecure(),
			grpc.WithChainUnaryInterceptor(ClientInterceptor(c.ServiceName)),
...
func ClientInterceptor(serviceName string) grpc.UnaryClientInterceptor {
	return func(ctx context.Context, method string, req, reply interface{}, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
		...
		if err != nil {
			return errors.Wrapf(err, "[grpc.client] trace inject span error :%v", err)
		} else {
