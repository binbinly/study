...
func (r *Repo) GetCategoryNameByID(ctx context.Context, id int64) (name string, err error) {
	if err = r.QueryCache(ctx, buildCategoryCacheKey(id), &name, func(data interface{}) error {
		// 从数据库中获取
...
func (r *Repo) GetCategoryNamesByIds(ctx context.Context, ids []int64) (names map[int64]string, err error) {
	...
	if err = r.Cache.MultiGetCache(ctx, keys, cacheMap, nil); err != nil {
...
func (r *Repo) CategoryAll(ctx context.Context) (list []*model.CategoryModel, err error) {
	if err = r.QueryCache(ctx, "category_all", &list, func(data interface{}) error {
		// 从数据库中获取
		if err := r.DB.WithContext(ctx).Model(&model.CategoryModel{}).Order(orm.DefaultOrderSort).Find(&list).Error; err != nil {
			return errors.Wrap(err, "[repo.category] query db")
		}
		return nil
	}); err != nil {
