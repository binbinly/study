...
func (r *Repo) GetSkuByID(ctx context.Context, id int64) (sku *model.SkuModel, err error) {
	if err = r.QueryCache(ctx, buildSkuCacheKey(id), &sku, func(data interface{}) error {
		// 从数据库中获取
		if err := r.DB.WithContext(ctx).First(data, id).Error; err != nil && err != gorm.ErrRecordNotFound {
			return errors.Wrapf(err, "[repo.sku] query db")
		}
		return nil
	}); err != nil {
...
func (r *Repo) GetSkusBySpuID(ctx context.Context, spuID int64) (list []*model.SkuModel, err error) {
	...
	if err = r.QueryCache(ctx, doKey, &list, func(data interface{}) error {
		// 从数据库中获取
		if err := r.DB.WithContext(ctx).Model(&model.SkuModel{}).Where("spu_id=?", spuID).Find(&list).Error; err != nil {
