...
func (r *Repo) GetSkuByID(ctx context.Context, skuID int64) (*model.SkuModel, error) {
	id := parseID(skuID)
	//获取商品所在场次
	...
	sku := &model.SkuModel{}
	err = json.Unmarshal([]byte(data), sku)
	if err != nil {
...
func (r *Repo) GetSkusBySessionID(ctx context.Context, sessionID int64) ([]*model.SkuModel, error) {
	data, err := r.redis.HGetAll(ctx, buildSkuKey(sessionID)).Result()
	if err != nil {
		return nil, errors.Wrapf(err, "[repo.sku] getall by sid: %v", sessionID)
	}
	var skus []*model.SkuModel
	for _, datum := range data {
		if datum == "" {
			continue
		}
		sku := &model.SkuModel{}
		err = json.Unmarshal([]byte(datum), sku)
		if err != nil {
