        ...
        <template v-for="(item,index) in cats">
          <div :id="'child-'+item.id">
            <template v-if="item.child.length > 0" v-for="(item2,index2) in item.child">
              <div class="m-1">
                <span class="d-block font-sm main-text-color" @click="openGoodsList(item2.id, item2.name)">{{item2.name}}</span>
              </div>
              <div class="row">
...
<script>
import { categoryTree } from '@/api/goods'
export default {
  data() {
    return {
      // 当前选中的分类
  ...
  methods: {
    ...
    openGoodsList(cat, title) {
      this.$router.push({ path: '/goods_list', query: { cat, title } })
    },
    onChange(index) {
