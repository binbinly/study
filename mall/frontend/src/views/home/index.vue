  ...
  <div>
    <van-sticky>
      <van-search show-action placeholder="请输入搜索关键词" @click="openSearch">
        <template #action>
          <span class="iconfont icon-richscan_icon" @click="event"></span>
        </template>
        <template #left>
      ...
      <van-tab v-for="(item,index) in tabBars" :title="item.name">
        <template v-if="item.list.length > 0">
          <template v-for="(list,listIndex) in item.list">
            <!-- 轮播图组件 -->
            <swiper-image v-if="list.type === 1" :images="list.data" />

            <!-- 首页分类 -->
            <index-nav v-else-if="list.type === 2" :resdata="list.data" />

            <!-- 三图广告 -->
            <three-adv v-else-if="list.type === 3" :resdata="list.data" />

            <!-- 大图广告位 -->
            <card v-else-if="list.type === 4" :headTitle="list.data.title" :bodyCover="list.data.cover" />
            ...
            <template v-else-if="list.type === 5">
              <van-list v-model="goodsList[index].loading" :finished="goodsList[index].finished" finished-text="没有更多了"
                        @load="onLoad(list.data.router)" error-text="加载失败，请重试">
                <div class="row j-sb">
...
import { homeData, homeCatData } from '@/api/home.js';
...
export default {
  ...
  methods: {
    ...
    getList(router) {
      ...
      api.post(router, { cid: this.tabBars[this.active].id }, page, false, true).then((res) => {
