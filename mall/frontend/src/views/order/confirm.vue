...
export default {
    ...
    getMyCoupon().then(res => {
      res.forEach(item => {
        let coupon = {
          id: item.id,
          condition: item.desc,
          value: item.type == 2 ? this.total - (item.value * this.total / 10) : this.value * 100,
          name: item.name,
          startAt: item.start_at,
          endAt: item.end_at,
          valueDesc: item.value,
          unitDesc: item.type == 1 ? '元' : '折',
        }
        if (this.total < item.min_price * 100) {
  ...
  methods: {
    ...
    submitCartOrder() {
      ...
      submitOrder({ coupon_id, address_id, remark: this.message, ids }).then(id => {
        console.log('id', id)
    ...
    submitGoodsOrder() {
      submitGoodsOrder({
        goods_id: this.items[0].goods_id,
        sku_id: this.items[0].sku_id || 0,
