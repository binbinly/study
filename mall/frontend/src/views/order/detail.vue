<template>
  <div>
    ...
    <van-divider />
    <order-goods :items="order.goods"></order-goods>
    <van-divider />
    <van-cell title="商品总价" :value="'￥'+order.total_price" />
    ...
    <van-cell-group title="订单信息">
      <van-cell title="创建时间" :value="order.created_at|formatTime" />
      <van-cell title="订单号" :value="order.order_no" />
      <van-cell title="订单备注" :value="order.user_note" />
    </van-cell-group>
